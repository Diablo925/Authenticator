...
if (isset($_POST['inotp'])) {
	if (ctrl_options::GetSystemOption('login_csfr') == 'false')
      runtime_csfr::Protect();
		
	require_once 'GoogleAuthenticator.php';
	$ga = new PHPGangsta_GoogleAuthenticator();
	$sql = $zdbh->prepare("SELECT ac_otpkey_vc FROM x_accounts WHERE ac_user_vc = :username AND ac_deleted_ts IS NULL");
	 $sql->bindParam(':username', $_SESSION["inusername"]);
    $sql->execute();
	$result = $sql->fetch();
	$secret = $result['ac_otpkey_vc'];
	$checkResult = $ga->verifyCode($secret, $_POST['inotp'], 2);
	if ($checkResult) {
	if (!ctrl_auth::Authenticate($_SESSION["inusername"], $_SESSION["secure_password"], $_SESSION["rememberdetails"], false, $_SESSION["inSessionSecuirty"])) {
        header("location: ./?invalidlogin");
        exit();
		}
	} else { 
	header("location: ./?otp&invalidtoken");
	exit();
	}
}
...
if (isset($_POST['inUsername'])) {
    ...
    $sql = $zdbh->prepare("SELECT ac_passsalt_vc, ac_otp_vc FROM x_accounts WHERE ac_user_vc = :username AND ac_deleted_ts IS NULL");
    ...
    $secure_password = $crypto->CryptParts($crypto->Crypt())->Hash;
	$otp = $result['ac_otp_vc'];
	if($otp == 1) { 
	$_SESSION["secure_password"] = $secure_password;
	$_SESSION["rememberdetails"] = $rememberdetails;
	$_SESSION["inSessionSecuirty"] = $inSessionSecuirty;
	$_SESSION["inusername"] = $_POST['inUsername'];
	 header("location: ./?otp");
	 exit();
	 } else {
